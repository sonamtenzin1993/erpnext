...
export default class ChartWidget extends Widget {
	...
	async render() {
		...
		// 	}
		// 	if (window.ApexDashboardRenderer) {

		// 		console.log(
		// 			"Using ApexCharts for Dashboard Chart:",
		// 			this.chart_doc.name
		// 		);

		// 		this.dashboard_chart =
		// 			window.ApexDashboardRenderer.render(
		...
		// 		console.warn(
		// 			"ApexDashboardRenderer not loaded. Using Frappe Chart."
		// 		);
    ...
    if (
        ...
        typeof window.ApexDashboardRenderer.render === "function"
    ) {
        const container = this.chart_wrapper?.[0];

        if (!container) {
            console.error(
                "ApexCharts: chart container not found"
            );
            return;
        }
    ...
    console.error(
        "ApexDashboardRenderer is not loaded"
    );
};
