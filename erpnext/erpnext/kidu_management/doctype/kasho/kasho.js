...
frappe.ui.form.on("Kasho", {
    ...
    registration_no(frm) {
        ...
        frappe.call({
            method: "erpnext.kidu_management.doctype.kasho.kasho.get_kasho_member_data",
            args: {
                registration_no: frm.doc.registration_no
            },
            callback: function (r) {
                ...
                // ✅ SAFE: replace whole child table at once
                frm.set_value("kidu_recipient", r.message);
                frm.refresh_field("kidu_recipient");
...
frappe.ui.form.on("Award and Appointment", {
    cid(frm, cdt, cdn) {
        ...
        if (row.cid && (row.cid.length === 11 || row.cid.length === 12)) {
            ...
            frappe.call({
                ...
                callback: function (r) {
                    ...
                    frappe.msgprint({
                        title: "Found",
                        message: `Record of ${r.message.registry_name} with CID ${row.cid} found.`,
                        indicator: "green"
                    });
                }
            });
